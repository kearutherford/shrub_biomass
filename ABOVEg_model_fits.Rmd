...
```{r}
# Note: CA.m2 in original dataframe was calculated incorrectly (it was in m^4)
# except for Battles and Harrington 

shrubs_2 <- shrubs_1 %>%
  rename(CA.m4 = CA.m2) %>%
  mutate(CA.m2 = if_else(Author == "Battles" | Author == "Harrington", CA.m4, pi*(CW1.m/2)*(CW2.m/2)),
         lnCA = log(CA.m2),
         lnAGL = log(AGL.g),
         lnHT = log(Hgt.m))
